...
class Metric():
    def __init__(self,GT,preHSI) -> None:
        ...
        self.PSNR,self.RMSE,self.SAM,self.ERGAS,self.SSIM,self.QIQC = [],[],[],[],[],[]
        ...
        if len(GT.shape) == 4:
            for i in range(len(self.GT)):
                ...
                # self.QIQC.append(self.calc_qiqc(self.GT[i],self.preHSI[i]))
            ...
            # self.QIQC = np.array(self.QIQC).mean()
        if len(GT.shape) == 3:
            ...
            # self.QIQC = self.calc_qiqc(self.GT,self.preHSI)
    ...
    @torch.no_grad()
    def calc_qiqc(self, GT_image, fuse_image, window_size: int = 8):
